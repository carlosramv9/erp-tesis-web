...
const GiveLoan = ({ setShow }) => {
  ...
  const processData = async (data, e) => {
    ...
    if (token) {
      ...
      dispatch(addBankMovementAction(bank._id, movement, data.toUser ? 'complete' : 'pendient'))
      setShow(false)
    ...
    <form onSubmit={handleSubmit(processData)} method="post" className='w-100 mb-5'>
      ...
      <div className="w-100 row">
        <div className="mb-3 col-md-3 col-sm-12">
          <label htmlFor="" className="form-label"><span className="text-white color-primary h5">*</span></label>
          <select className="form-select" name='to' placeholder="Seleccionar..." value={type} onChange={(e) => setType(e.target.value)}>
            <option value={'toUser'}>Personal</option>
            <option value={'toBank'}>Banco</option>
          </select>
          <span className='text-danger text-small d-block my-2'>{errors?.to?.message}</span>
        </div>
      ...
      <div className="d-flex justify-content-center">
        <div className="mb-3 text-center">
          ...
          <input type="number" className="form-control col-md-3 col-sm-12" name='amount' defaultValue={0} min={0} {...register("amount", { required: { value: true } })}
